...
public class ArraysAreEqual {
    ...
    public boolean equal(int[] a, int[] b){
        if(a.length != b.length){ // first check if the arrays are even the same length if not automatically cant have equal elemets
            return false;
        }
        for( int i = 0; i < a.length; i++){ // Loop through an arr length and check if numbers compare to array b
            if (a[i] != b[i]){
                return false;
            }
        }
